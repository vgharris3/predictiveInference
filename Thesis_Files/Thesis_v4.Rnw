...
xlabel = eval(bquote(expression(tilde(t)*" = Number of Future Successes out of "*.(M))))
...
res = hist(random_sample, prob = TRUE, breaks = breaks,main = "Histogram of Sample with Probability Mass Overlay",ylim = c(0,max(f_t)*1.5),xlim = c(0,M),xlab = xlabel,ylab = "Count")

# lines(0:M,f_t)
points(0:M,f_t, col = "red", pch = 16)
