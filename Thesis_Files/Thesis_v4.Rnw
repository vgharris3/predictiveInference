...
\clearpage
  ...
  \textcolor{red}{why the ff after the page number?}\textcolor{blue}{That just means ``and following".  I could change that to pp. 43-50}
...
\noindent The user provides the two samples $\mathbf{y_1}$ and $\mathbf{y_2}$ along with values for $\mu_0, \sigma^2_0, \delta_0, \tau^2_0, \nu_0$, and desired prediction sample size $M$.  The function returns $M$ predictions for each population and the vectors of generated values for $\mu$, $\delta$, and $\sigma^2$.  Calls to this functions appear as follows:
