...
I find persuasive the assertion that the main purpose of statistics is to predict future events based on observed data, and moreover that a statistical model may be judged chiefly by the quality of its predictions.  Prediction about meaningful quantities that are relevant to the object of study facilitates scientific progress in multiple ways.

Billheimer underscores the improvement of scientific accuracy and reproducibility to be had by ``shifting our focus from `finding differences' among hypothetical parametrs to predicting observable events based on our current scientific understanding.

enabling corroboration or refutation of current hypotheses through future experimentation, informing decision-making by summarizing quantities of direct interest to the researcher, and shifting the focus of statistical analysis from estimation of hypothetical parameters to statements about concrete observables.\\

\noindent It is not the intent of this thesis to suggest that parametric inference should be abandoned in statistical analyses.  Important uses for conventional statistical inference techniques include summarizing information about large quantities of data in a handful of usable values, and leveraging such summaries to determine whether a particular problem merits continued attention.  In favor of Bayesian methods, I return to Nate Silver:\\
\begin{quote}
...
\end{quote}
%% dean - this last sentence is not true. Bayesian inference predated
%% frequentist. Instead, how about, ``... conventional statistical inference
%% techniques are useful for summarizing (?concentrating?) information about
%% large quantities of data in a small number of parameter estimates, .....,
%% Indeed, the scientific discipline of mathematical statistics has developed along
%% parametric lines, imparting greater efficiency in solving idealized
%% versions of problems. However, this focus on parameters has largely ignored the
%% impact on observable quantities which may exhibit distributions very
%% different from our standared models.  Or something like that.

\noindent Prediction is a means of discriminating between scientific hypotheses.

Generally, a model may be judged by the quality of its predictions.  Given competing models, the better predictor will be given more weight, and a useful model increases in utility as its predictive capability improves.

%% dean - you need to finish the section on discriminating scientific
%% hypotheses. As written, it just stops.
%% You also need a transition ?a paragraph? to Bayesian prediciton vs. plug-in estimators.
...
%% how about a picture of a pig in razorback position?  e.g. via cell phone?
...
%% can you add a ruler or some other scale figure to show relative size?
\begin{figure}[ht]
  \centering
...
%% dean - in the paragraph above - are these shape factors or scale factors?
...
%% dean - write about your conclusion from the Big pig experiment and what it
%% has to say about preditcion????
%%  this is a really nice example, but you need to motivate it better - e.g. the
%%  note above, and you need to have a conclusion that describes the benefits of
...
is likely to be the value of the next observation,
%% dean  - this needs to be just $\tilde{Y}$ DONE
$\tilde{Y}$?
%%=\tilde{y}$?
We want to compute $Pr(\tilde{Y}|y_1,...,y_n)$, where $y_1,...,y_n$ are
conditionally independent and identically distributed (i.i.d.)
%% dean with respect to DONE
with respect to
%conditional on
 a population parameter (or parameters) $\theta$.  We assign prior distribution
 $\pi(\theta)$ based on some existing knowledge or beliefs.  Here we are careful
 to satisfy ourselves that $Y_1,...,Y_n$ are \textit{exchangeable}, which
 enables us to rely on de Finetti's representation theorem for the
%% dean
 \textcolor{red}{dean i.i.d.  -- also cite de Finetti's theorem e.g. from Hoff's book.
  You probably want to state the definition of exchangeability, and use that
  throughout the thesis, rather than the iid description.}

 conditional independence assumption.  Thus we can write
...
%% dean -
\textcolor{red}{so what does equation 1 tell you? describe this result}\\

\textcolor{red}{for the sentence below - isn't it characterizing theta given the observed data?}\\
...
%% dean -
\textcolor{red}{I think the paragraaph above needs a little more explanation.}

\clearpage

\section{Predictive Problems with Conjugate Priors}

\noindent When data can be modeled with a distribution that
%% dean suggests DONE
suggests
%admits %%
a conjugate prior for the parameter(s) of interest, and those parameters can
reasonably be represented by that conjugate prior, the posterior calculations
are greatly simplified.  The four classes of models addressed in this section
all exhibit this feature.  The first three are single-parameter exponential
families and have closed-form solutions for prediction.  The fourth is a
two-parameter exponential family with unkown mean and variance.  While this one
does not admit an analytical solution, prediction is easily accomplished by
means of simple
%% dean monte carlo DONE
monte carlo %% it's a proper noun
(MC) sampling.  The four classes of predictive models presented in this section are:
\begin{itemize}
...
size, and $S$ is the size of the desired random sample of predictions \sout{number of predictions desired}. The predicted future result
is indicated by surmounting the
%% dean data
%random %% random variable??
variable name with a tilde.  For example, when the observed data is represented by $y$, the prediction is designated $\tilde{y}$.  %Parameter variable names are clearly identified as they appear.
Parameter names are clearly indicated as they appear.
%% dean ?? parameter variable?  what are you trying to say? Parameters are
%% defined as they appear?
  ...
  \subsection{Prediction of Future Successes:  Beta-Binomial (Geisser p. 73)}
    ...
    \noindent Let $Y_1,...,Y_N$ be
%% dean
conditionally \textcolor{red}{Check for this in the remainder of the thesis.  Observations
  are conditinally independent give some theta. - Or observations are
  ``modeled'' as exchangeable}\textcolor{blue}{Discuss:  Hoff defines exchangeability and cites De Finetti but then introduces all of his models with ``i.i.d."}

independent binary variables with Pr$(Y_i = 1) = \theta$, with $Y_i = 1$ indicating success and $Y_i = 0$ indicating failure.  The number of observed successes can be represented by $T = \sum Y_i$, which is sufficient for $\theta$ and has a binomial$(N,\theta)$ distribution.  That is,
...
\begin{flalign*}
  \texttt{tpred} &= \tilde{t} \text{, the number(s) of future successes for which prediction is desired}\\
  \texttt{N} &= \text{the number of observations}\\
  \texttt{t} &= \text{the observed number of successes out of M future observatioins}\\
  \texttt{M} &= \text{the number of future observations}\\
...
%% dean - you need to define S in your argument list DONE

    \subsubsection{Example}

The functions \texttt{dpredBB()} and \texttt{rpredBB()} were used in the Pass The Pigs\textsuperscript{\circledR} example in the introduction.  Here, all three functions are exercised in an imagined experiment using the Big Pigs\texttrademark variant.  Suppose $t=7$ Razorbacks have been observed out of $n=10$ tosses of Big Pigs\texttrademark, and the researcher has settled on prior distribution Pr(Razorback) = $\theta\sim\text{Beta}(2,8)$.  For $N = 1000$ future observations, how many successes are predicted?  The figures below show the predictive distribution from \texttt{dpredBB()}, the cumulative distribution from \texttt{ppredBB()}, and a histogram of random draws from \texttt{rpredBB()}.
...
S = 10000 # random sample size desired
...
xlabel = eval(bquote(expression(tilde(t)*" = Number of Future Successes out of 1000")))

plot(tpred,f_t,type = "l", col = "blue", lwd = 2, main = "Beta-Binomial Predictive Density",xlab = xlabel,ylab = "dpredBB()")

#Cumulative predictive probability
#set.seed(2)
F_t = ppredBB(tpred,N,t,M,a,b)
plot(tpred,F_t,type = "l", col = "red", lwd = 2, main = "Beta-Binomial Cumulative Predictive Probability",xlab = xlabel,ylab = "ppredBB()")

#Random sample
#set.seed(3)
f_t = dpredBB(0:M,N,t,M,a,b)
random_sample = rpredBB(S,N,t,M,a,b)
breaks = seq(-0.5,M+0.5,1)

res = hist(random_sample, prob = TRUE, main = "Histogram of Sample with Density Curve Overlay",ylim = c(0,max(f_t)*1.5),xlim = c(0,M),xlab = xlabel,ylab = "Count")
...
% \textcolor{red}{explain context}

    \subsubsection{Derivation}
%% dean -

\textcolor{red}{the work copies is kind of weird here?  realizations? observations?}
...
%% dean
\textcolor{red}{I think you are confusing y - the observation, with the censoring value
ususally this would be someting like Pr(Y > t | $\theta$) where t is the
 censoring value.}
...
%% dean
\textcolor{red}{this is a continuous preductuve density, we would yusually write this as $p(\tilde{y} | Y_1, ..)$}
    \begin{flalign}
...
set.seed(1234)
th = .01
y = rexp(N,th) #vector of observed survival times
tcutoff = 160 #experiment time cutoff
y[y > tcutoff] = tcutoff
c = numeric(N)
c[y < tcutoff] = 1
d = length(y[y < tcutoff]) # number of fully observed copies
...
ypred = 1:1000 #cutoff times for which prediction is desired

f_y = dpredEG(ypred, y, c, dt, gm)

xlabel = eval(bquote(expression(tilde(y)*" = Survival Time")))
...
random_sample = rpredEG(S, y, c, dt, gm)
hist(random_sample,prob = TRUE,main = "Histogram of Sample with Density Curve Overlay", ylim = c(0,max(f_y)*1.1),xlab = xlabel,ylab = "Count")
lines(1:1000,f_y)
...
\clearpage
  ...
  \textcolor{red}{why the ff after the page number?}\textcolor{blue}{That just means ``and following".  I could change that to pp. 43-50, or get rid of this citation and just call it out below?}

    \subsubsection{Derivation}

\textcolor{red}{Again - iid or conditionally independent. de Finetti uses the term exchangeable
Why not this?}\textcolor{blue}{Hoff p. 46}
...
\textcolor{red}{you dropped $d\theta$ from the integral 2nd 3rd and 4th lines of equation}\textcolor{blue}{and line 1 of the first equation.  Double-check.}
...
set.seed(1234)
y = c(27, 79, 21, 100, 8, 4, 37, 15, 3, 97)
ypred = 0:60
alpha = 11
beta = 3
par(mfrow=c(3,1))
...
xlabel = eval(bquote(expression(tilde(y)*" = Future Occurences")))

plot(1:length(f_y),f_y,type = "l", col = "blue", lwd = 2, main = "Poisson-Gamma Predictive Density", xlab = xlabel, ylab = "dpredPG()")

#Cumulative predictive probability
F_y = ppredPG(ypred, y, alpha, beta)
plot(1:length(F_y),F_y,type = "l", col = "red", lwd = 2, main = "Poisson-Gamma Cumulative Predictive Probability", xlab = xlabel, ylab = "ppredPG()")

#Random sample
S = 100000 #random sample size
random_sample = rpredPG(S,y, alpha, beta)
...
res = hist(random_sample,prob = TRUE, main = "Histogram of Sample with Density Curve Overlay",ylim = c(0,max(f_Rand)*1.1),breaks = breaks, xlab = xlabel, ylab = "Count")
...
\clearpage
  ...
  \subsection{Normal Observation with Normal-Inverse Gamma Prior}
    ...
    \subsubsection{One sample Normal-Inverse Gamma}
      ...
      [Hoff p. 69ff]\\
      \textcolor{red}{dean-again, why the ff?}\\
      \textcolor{blue}{change to (Hoff Chapter 5)?  Or get rid of this citation because I cite Hoff by page number below, as well as Gelman?}
...
\end{flalign*}
      ...
      Math score data for two schools (we'll call them School 1 and School 2) were based on results of a national exam in the United States, standardized to produce a nationwide mean of 50 and a standard deviation of 10.  The data consisted of 31 individual scores for School 1 and 28 for School 2.  We're interested in predicting a future math score by a student from each school.  Unless the two schools were known in advance to be extremely exceptional, reasonable prior parameters can be based on this information.  (The mean scores for Schools 1 and 2 were 50.8 and 46.2, and the standard deviations were 11.3 and 9.1, respectively).  For the prior distributions of $\mu$ and $\sigma^2$, we'll take $\mu_0 = 50$ and $\sigma^2_0 = 10^2 = 100$, although this latter value is likely to be an overestimate of the within-school sampling variability.  We'll make these prior distributions somewhat diffuse, with $\gamma^2_0 = 25^2 = 625$ and $\nu_0 = 1$.  For the prior distribution on $\delta$, choosing $\delta_0 = 0$ represents the prior opinion that $\theta_1 > \theta_2$ and $\theta_2 > \theta_1$ are equally probable.  Finally, since the scores are bounded between 0 and 100, half the difference between $\theta_1$ and $\theta_2$ must be less than 50 in absolute value, so a value of $\tau^2_0 = 25^2 = 625$ seems reasonably diffuse.\\\\

\textcolor{red}{Are there data for example 3.4.2.3? You haven't described any. Maybe a plot?
Also - what is being predicted here? Is it a future single student from each of
the selected schools?}\textcolor{blue}{See additional words added above--does this suffice?  Or do should I also include a plot?}
...
\begin{flalign*}
  \texttt{S} &= S \text{, the desired random sample size}\textcolor{blue}{\text{replace with next line?}}\\
  \texttt{S} &= S \text{, the number of MCMC realizations to be generated}\\
...
\begin{flalign*}
  \texttt{S} &= S \text{, the desired random sample size}\textcolor{blue}{\text{replace with below}}\\
  \texttt{S} &= S \text{, the number of MCMC realizations}\\
