...
res = hist(random_sample,prob = TRUE, main = "Histogram of Sample with Probability Mass Overlay",xlim = c(0,length(F_y)),ylim = c(0,max(f_Rand)*1.1),breaks = breaks, xlab = xlabel, ylab = "Count")
...
boxplot(list(y1,y2),range=0,col=c("red","blue"),ylab="score",names=c("school 1","school 2"))
