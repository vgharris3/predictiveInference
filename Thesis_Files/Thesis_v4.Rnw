...
Suppose we are told by a hurricane expert that the average number per decade can be expected to be around 4.  The historic data suggests about a 2-year gap between large hurricane landfalls on average.  Based on this information we choose $\alpha = 8$ and $\beta = 2$ for the Gamma$(\alpha,\beta)$ prior on $\theta$.  For $\tilde{y} = 1:20$ possible large hurricanes making landfall in the U.S. in the 2020s, Figure \ref{fig:PGdist} below shows the predictive distribution from \texttt{dpredPG()}, the cumulative distribution from \texttt{ppredPG()}, and a histogram of a predictive sample of size 1000 returned by \texttt{rpredPG()}.
...
res = hist(pred_sample,prob = TRUE, main = "Histogram of Predictive Sample",xlim = c(0,length(F_y)),breaks = breaks, xlab = xlabel, ylab = "Rel. Freq.")
