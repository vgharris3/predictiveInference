...
YN = sort(rexp(N,th))
tcutoff = 160
YN[YN > tcutoff] = tcutoff
# d = 800
d = length(YN[YN < tcutoff])
# YNminusd = YN
# YNminusd[(d+1):N] = YN[d] #last N - d copies are censored
#
# if (plotflag1){
#   plot(1:N,YN)
#   lines(1:N,YNminusd)
...
f_y = dpredEG(y, YN, d, dt, gm)

xlabel = eval(bquote(expression(tilde(y)*" = Survival Time")))

plot(y[1:plot_length],f_y[1:plot_length],type = "l", lwd = 2, col = "blue", main = "Exponential-Gamma Predictive Density",xlab = xlabel,ylab = "dpredEG()")

F_y = ppredEG(y, YN, d, dt, gm)

plot(y[1:plot_length],F_y[1:plot_length],type = "l", lwd = 2, col = "red", main = "Exponential-Gamma Cumulative Predictive Probability",xlab = xlabel, ylab = "ppredEG()")

n = 1000

random_sample = rpredEG(n, YN, d, dt, gm)
...
#Predictive density
f_y = dpredPG(ytilde, obs, alpha, beta)
...
#Cumulative predictive probability
F_y = ppredPG(ytilde, obs, alpha, beta)
