...
\begin{center}
  \texttt{dpredEG(ypred,y,d,dt,gm)}\\
  \texttt{ppredEG(ypred,y,d,dt,gm)}\\
  \texttt{rpredEG(S,y,d,dt,gm)}\\
\end{center}
...
\begin{flalign*}
  ...
  \texttt{d} &= \text{the number of censored survival times}\\
...
random_sample = rpredEG(S, y, d, dt, gm)
...
\noindent Calls to these functions will appear as follows:
