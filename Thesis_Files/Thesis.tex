    ...
    \end{flalign*}
      ...
      \begin{enumerate}
        ...
        \item For $s\in\{1,...,S\}$, sample
          ...
          \end{enumerate}
        \item For $s\in\{1,...,S\}$, generate prediction $y_j^{(s)} \sim \text{normal}\left(\theta_j^{(s)},\sigma^{2(s)}\right)$
      \end{enumerate}
...
\includegraphics{Thesis-008}
      ...
      We'll use the following prior values (Hoff p. 137):
...
NEXT PUT STEPS 1-4 (HOFF P. 137) INTO ABOVE R IMPLEMENTATION SECTION
