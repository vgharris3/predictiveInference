...
matplot(x,fmat,type="l",lty=c(1,2,3,1),lwd = 2, col = c(1,1,1,2), ylim = c(0,ymax),
     ...
     xlab = "Number of Razorbacks out of 100 Pig Tosses", ylab = "Density")
...
# par(mfrow = c(2,1))
#
# hist(random_sample, prob = TRUE, main = "Histogram of Sample with Density Curve Overlay",ylim = c(0,max(f_x)*1.5),xlim = c(0,m))
# lines(0:m,f_x)
#
# hist(rs2, prob = TRUE, main = "Histogram of Sample with Density Curve Overlay",ylim = c(0,max(f_x)*1.5),xlim = c(0,m))
# lines(0:m,f_x)
...
################################################
##drawing new theta from posterior for each prediction
  ...
  # par(mfrow=c(1,2))
  #
  # hist(random_sample,breaks=seq(0,100,5),prob = TRUE,main = "Histogram of Sample with Density Curve Overlay", ylim = c(0,max(f_x)*1.1))
  # lines(1:1000,f_x)
  # hist(rs2,breaks=seq(0,100,5),prob = TRUE,main = "Histogram of Sample with Density Curve Overlay", ylim = c(0,max(f_x)*1.1))
  # lines(1:1000,f_x)
...
The density function \texttt{dpredPG()} simply makes use of the R function \texttt{dnbinom()}.  The cdf \texttt{ppredPG()} returns a cumulative sum of the results of \texttt{dpredPG()}.  The random sampler \texttt{rpredPG()} is a bit more complicated. The difficulty is that the upper bound of the support of the predictive distribution $p(\tilde{y}|y_1,...,y_n)$ is not known.  Since $p(\cdot)$ is negative binomial, we can count on it approaching $0$ at the upper end.  To establish the support, then, a method was employed to find where $p$ comes ``sufficiently close" to $0$.  To accomplish this a modified bisection method was devised as follows:\\

% Initially the R function \texttt{uniroot()} was considered, but ultimately a modified bisection method was devised as follows:\\

    \begin{enumerate}
      \item Set a desired tolerance $\epsilon$ for the distance of the predictive distribution above zero at the upper end of its support.  Currently the function leverages the local device's numerical precision with $\epsilon = $ \texttt{.Machine\$double.eps}\textcolor{red}{fix this}%$\$$\texttt{double.eps^0.5}.
      \item Set lower bound $L$ equal to the expected value of $\tilde{Y}$.  that is, $L = E(\tilde{Y}|y_1,...,y_N) = \dfrac{\alpha+\sum{y_i}}{\beta+N}$ (negative binomial). \textcolor{red}{is L guaranteed to be an integer here?  Does it matter?}
      \item Step to the right of $L$ by increments in the sequence $L+1,L+2,L+4,...,L+2^k,...$, setting upper bound $U_{test} = 2^k$ with the first value of $k$ for which $\texttt{dpredPG}\left(L + 2^k\right) < 0$.
      \item Bisect the interval $[L,U_{test}]$ letting $B$ be the integer nearest to the middle of the interval.
      ...
      \item Establish a new interval $[L',U'_{test}]$ as follows:
        \subitem if $[\texttt{dpredPG}(L),\texttt{dpredPG}(B)]$ straddles $0$, then $[L',U'_{test}] = [L,B]$.
        \subitem if $[\texttt{dpredPG}(B),\texttt{dpredPG}(U_{test})]$ straddles $0$, then $[L',U'_{test}] = [B,U_{test}]$.
      \item Repeat steps 3 - 5 above with the updated interval $[L,U_{test}] = [L',U'_{test}]$ until the condition in step 5 is reached.
...
for(i in 1:length(mu0)){
  random_sample_MC[,i] = rpredNormIG1(S,y,mu0[i],k0,sig20,nu0)
...
random_sample_J = rpredNormIG1(S,y,Jeffreys=TRUE)
...
xd = dpredNormIG1(x,y,mu0[pos1.9],k0,sig20,nu0,Jeffreys=FALSE)
plot(x,xd,type="l",col="blue",lwd=2,main=plotTitle_dp,xlab="y",ylab = "dpredNormIG1(y)",xlim=c(1,3))

xd_J = dpredNormIG1(x,y,Jeffreys=TRUE)
...
xp = ppredNormIG1(x,y,mu0[pos1.9],k0,sig20,nu0,Jeffreys=FALSE)
plot(x,xp,type="l",col="blue",lwd=2,main=plotTitle_pp,xlab="y",ylab = "ppredNormIG1(y)",xlim=c(1,3))

xp_J = ppredNormIG1(x,y,Jeffreys=TRUE)
...
abs_diffs = abs(ybar - overall_mean) #absolute diff between schools' averages and overall average
...
\begin{center}
  \begin{tabular}{|c|c|c|}
    ...
    \hline
    overall mean & -- & \Sexpr{round(overall_mean,2)} \\
    \hline
...
result = rpredNormIGk(N=5000,Y.school.mathscore,nu0=1,s20=100,eta0=1,t20=100,mu0=50,g20=25)
...
for(legInd in 1:length(toplot)){
  nextlegObs = eval(bquote(expression(y*" (School "*.(toplot[legInd])*")")))
  nextlegPred = eval(bquote(expression(tilde(y)*" (School "*.(toplot[legInd])*")")))
  if(legInd == 1){
...
\begin{flalign*}
  ...
  S &= \Sexpr{S} \text{ (sample size for predictive distribution random draw)}
\end{flalign*}
...
################
################
#THIS BLOCK REPEATS THE SETUP FROM PREVIOUS CHUNK
...
Xpred = X          #predictor vector(s) for which prediction is desired
...
result = rpredNormReg(S,Xpred,X,y,beta.0,Sigma.0,nu.0,sig20,gprior=FALSE)
result2 = rpredNormReg(S,Xpred,X,y,beta.0,Sigma.0,nu.0,sig20,gprior=FALSE)
result3 = rpredNormReg(S,Xpred,X,y,beta.0,Sigma.0,nu.0,sig20,gprior=FALSE)

maxx = 0
minx = 0
maxy = 0
numplots = nrow(result$predictions)
legtxt = vector(mode = "character",length = numplots)
for(i in 1:numplots){
  tempmaxx = max(density(result$predictions[i,])$x)
  if(tempmaxx > maxx) {maxx = tempmaxx}
  tempminx = min(density(result$predictions[i,])$x)
  if(tempminx < minx) {minx = tempminx}
  tempmaxy = max(density(result$predictions[i,])$y)
  if(tempmaxy > maxy) {maxy = tempmaxy}
  legtxt[i] = toString(X[i,])
}

plot(density(result$predictions[1,]), main = bquote("Predictive Density Using Semi-Conjugate Prior"),col=1,xlim = c(minx,1.25*maxx),ylim = c(0,1.1*maxy))


for(i in 2:numplots){
  lines(density(result$predictions[i,]),col=i)
}

legend("topright",legend = legtxt,lty = 1, col=1:numplots,bty = "n",title=expression("X"[1]*", "*"X"[2]*", "*"X"[3]*", "*"X"[4]))

for(i in 1:numplots){
  lines(density(result2$predictions[i,]),lty=2,col=i)
}

for(i in 1:numplots){
  lines(density(result3$predictions[i,]),lty=3,col=i)
}
...
%For the same example without prior information, we use Zellner's g-prior with $g =$ length$(y) =$ \Sexpr{length(y)}.
...
xp1 = rep(1,8)
xp2 = c(rep(0,4),rep(1,4))
xp3 = rep(c(20,24,28,32),2)
xp4 = xp2*xp3
...
#       result = rpredNormReg(S,CurrentX,X,y,nu0=nu0,s20=sig2.ols)
result = rpredNormReg(S,Xpred,X,y,nu0=nu0,s20=sig2.ols)
result2 = rpredNormReg(S,Xpred,X,y,nu0=nu0,s20=sig2.ols)
result3 = rpredNormReg(S,Xpred,X,y,nu0=nu0,s20=sig2.ols)
...
for(i in 1:numplots){
  ...
  if(tempmaxy > maxy) {maxy = tempmaxy}
  # if(Xpred[i,2]){
  #   legtxt[i] = paste("Aerobic, age ",toString(Xpred[i,3]))
  # }else{
  #     legtxt[i] = paste("Running, age ",toString(Xpred[i,3]))
  #   }
  legtxt[i] = toString(Xpred[i,])
}

plot(density(result$predictions[1,]), main = bquote("Predictive Density Using Zellner's g-prior"),col=1,xlim = c(minx,1.25*maxx),ylim = c(0,1.1*maxy),lwd = 2)

ncolor = nrow(Xpred)/2

for(i in 2:numplots){
  lines(density(result$predictions[i,]),lty=1,col=i)
}

# legtitle = paste(expression("X"[1]*", "*"X"[2]*", "*"X"[3]*", "*"X"[4]),"\nasdf")
# legtitle = expression(paste("X"[1]*", "*"X"[2]*", "*"X"[3]*", "*"X"[4]),"\nasdf")
# legend("bottomright",legend = legtxt,lty = 1, col=1:numplots,bty = "n",title=legtitle)
#
legend("topright",legend = legtxt,lty = 1, col=1:numplots,bty = "n",title=expression("X"[1]*", "*"X"[2]*", "*"X"[3]*", "*"X"[4]))
# legend("topright",legend = legtxt,lty = c(rep(1,ncolor),rep(2,ncolor)), lwd = 2, col=c(1:ncolor,1:ncolor),bty = "n")
...
for(i in 2:numplots){
  linecolor = i%%(ncolor)
  if(linecolor == 0){linecolor = ncolor}
  if(Xpred[i,2]){
    linetype = 2
  }else{
    linetype = 1
  }
  lines(density(result$predictions[i,]),col=linecolor,lty=linetype,lwd = 2)
}
...
Xpred = cbind(xp1,xp2,xp3,xp4)
y_obs = c(y23_0,y27_0,y23_1,y27_1)
...
for(i in 2:numplots){
  ...
  lines(density(result$predictions[i,]),col=linecolor,lty=linetype,lwd = 2)
  abline(v=y_obs[i],col=linecolor,lty=linetype,lwd=2)
}

legend("topright",legend = legtxt,lty = c(rep(1,ncolor),rep(2,ncolor)), lwd = 2, col=c(1:ncolor,1:ncolor),bty = "n")
text(0.8*minx,maxy,"Vertical lines show \n observed values \n for each case.")
...
minx = 0
maxy = 0
numplots = nrow(result_scp$predictions)
for(i in 1:numplots){
...
for(whichPredictor in 1:length(y)){

  Xpred = X[whichPredictor,]
  ...
  # plot(density(result_scp$predictions[whichPredictor,]), main = bquote("Semi-conjugate prior vs. Zellner's g-prior"),col=whichPredictor,xlim = c(minx,1.25*maxx),ylim = c(0,1.1*maxy))
  plot(density(result_scp$predictions), main="",col=whichPredictor,xlim = c(minx,1.25*maxx),ylim = c(0,1.1*maxy))
  ...
  #for(i in 1:numplots){
    # lines(density(result_zgp$predictions[whichPredictor,]),lty=2,col=whichPredictor)
    lines(density(result_zgp$predictions),lty=2,col=whichPredictor)
  #}

 # for(i in 1:numplots){
    abline(v=y[whichPredictor],col=whichPredictor,lty=1,lwd=2)
  #}
}
...
result_scp1 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=nu0,s20=sig20,gprior=FALSE)
result_scp2 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=diag(diag(Sigma.0)),nu0=nu0,s20=sig20,gprior=FALSE)
result_scp3 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=diag(4),nu0=nu0,s20=sig20,gprior=FALSE)
result_scp4 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=diag(rep(1000000,4)),nu0=nu0,s20=sig20,gprior=FALSE)
result_scp5 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=matrix(runif(16),ncol=4),nu0=nu0,s20=sig20,gprior=FALSE)
result_scp6 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=matrix(runif(16)*1000000,ncol=4),nu0=nu0,s20=sig20,gprior=FALSE)
result_zgp = rpredNormReg(S,Xpred,X,y,nu0=nu0,s20=sig2.ols)

lt1 = expression((X^T*X)^-1*sigma^2*n)
lt2 = expression(paste("diag"((X^T*X)^-1*sigma^2*n)))
lt3 = expression(I)
lt4 = expression(I%*%10^6)
lt5 = expression("Unif(0,1) 4"%*%"4 matrix")
lt6 = expression(paste("Unif(0,",10^6,") 4"%*%"4 matrix"))
lt7 = expression("Zellner's g-prior")
ltitle = expression(Sigma[0])

ptitle = expression(paste("Varying Prior Information:  ", Sigma[0]))
...
maxx = max(max(density(result_scp1$predictions[1,])$x),max(density(result_scp2$predictions[1,])$x),max(density(result_scp3$predictions[1,])$x),max(density(result_scp4$predictions[1,])$x),max(density(result_scp5$predictions[1,])$x),max(density(result_scp6$predictions[1,])$x),max(density(result_zgp$predictions[1,])$x))

minx = min(min(density(result_scp1$predictions[1,])$x),min(density(result_scp2$predictions[1,])$x),min(density(result_scp3$predictions[1,])$x),min(density(result_scp4$predictions[1,])$x),min(density(result_scp5$predictions[1,])$x),min(density(result_scp6$predictions[1,])$x),min(density(result_zgp$predictions[1,])$x))

maxy = max(max(density(result_scp1$predictions[1,])$y),max(density(result_scp2$predictions[1,])$y),max(density(result_scp3$predictions[1,])$y),max(density(result_scp4$predictions[1,])$y),max(density(result_scp5$predictions[1,])$y),max(density(result_scp6$predictions[1,])$y),max(density(result_zgp$predictions[1,])$y))

miny = min(min(density(result_scp1$predictions[1,])$y),min(density(result_scp2$predictions[1,])$y),min(density(result_scp3$predictions[1,])$y),min(density(result_scp4$predictions[1,])$y),min(density(result_scp5$predictions[1,])$y),min(density(result_scp6$predictions[1,])$y),min(density(result_zgp$predictions[1,])$y))

plot(density(result_scp1$predictions[1,]), main = ptitle,col=c(rep(1,6),2),xlim = c(minx,1.25*maxx),ylim = c(0,1.1*maxy))

#PLOT DIFFERENT RESULTS FOR THE DIFFERENT PRIOR COVARIANCE MATRICES.
lines(density(result_scp2$predictions[1,]),lty=2)
lines(density(result_scp3$predictions[1,]),lty=3)
lines(density(result_scp4$predictions[1,]),lty=4)
lines(density(result_scp5$predictions[1,]),lty=5)
lines(density(result_scp6$predictions[1,]),lty=6)
lines(density(result_zgp$predictions[1,]),col=2,lty=7,lwd=2)

legtxt = c(lt1,lt2,lt3,lt4,lt5,lt6,lt7)
legend(0.4*maxx,maxy,legend = legtxt,title = ltitle,col=c(rep(1,6),2),lty = 1:7,lwd=c(rep(1,6),2),bty = "n")
...
result_scp1 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=nu0,s20=sig20,gprior=FALSE)
result_scp2 = rpredNormReg(S,Xpred,X,y,beta0=2*beta.ols,Sigma0=Sigma.0,nu0=nu0,s20=sig20,gprior=FALSE)
result_scp3 = rpredNormReg(S,Xpred,X,y,beta0=3*beta.ols,Sigma0=Sigma.0,nu0=nu0,s20=sig20,gprior=FALSE)
result_scp4 = rpredNormReg(S,Xpred,X,y,beta0=4*beta.ols,Sigma0=Sigma.0,nu0=nu0,s20=sig20,gprior=FALSE)
result_scp5 = rpredNormReg(S,Xpred,X,y,beta0=5*beta.ols,Sigma0=Sigma.0,nu0=nu0,s20=sig20,gprior=FALSE)
result_scp6 = rpredNormReg(S,Xpred,X,y,beta0=6*beta.ols,Sigma0=Sigma.0,nu0=nu0,s20=sig20,gprior=FALSE)
result_zgp = rpredNormReg(S,Xpred,X,y,nu0=nu0,s20=sig2.ols)

lt1 = expression(beta[ols])
lt2 = expression(2*beta[ols])
lt3 = expression(3*beta[ols])
lt4 = expression(4*beta[ols])
lt5 = expression(5*beta[ols])
lt6 = expression(6*beta[ols])
lt7 = expression("Zellner's g-prior")
ltitle = expression(beta[0])

ptitle = expression(paste("Varying Prior Information:  Scaling ", beta[0]))
...
plot(density(result_scp1$predictions[1,]), main = ptitle,col=c(rep(1,6),2),ylim = c(0,1.2*maxy))
...
legtxt = c(lt1,lt2,lt3,lt4,lt5,lt6,lt7)
legend("topright",legend = legtxt,title = ltitle,col=c(rep(1,6),2),lty = 1:7,lwd=c(rep(1,6),2),bty = "n")
...
result_scp1 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=nu0,s20=sig20,gprior=FALSE)
result_scp2 = rpredNormReg(S,Xpred,X,y,beta0=rep(1,4),Sigma0=Sigma.0,nu0=nu0,s20=sig20,gprior=FALSE)
result_scp3 = rpredNormReg(S,Xpred,X,y,beta0=runif(4),Sigma0=Sigma.0,nu0=nu0,s20=sig20,gprior=FALSE)
result_scp4 = rpredNormReg(S,Xpred,X,y,beta0=rnorm(4),Sigma0=Sigma.0,nu0=nu0,s20=sig20,gprior=FALSE)
result_scp5 = rpredNormReg(S,Xpred,X,y,beta0=10*rnorm(4),Sigma0=Sigma.0,nu0=nu0,s20=sig20,gprior=FALSE)
result_scp6 = rpredNormReg(S,Xpred,X,y,beta0=100*rnorm(4),Sigma0=Sigma.0,nu0=nu0,s20=sig20,gprior=FALSE)
result_zgp = rpredNormReg(S,Xpred,X,y,nu0=nu0,s20=sig2.ols)

lt1 = expression(beta[ols])
lt2 = expression("("*1*", "*1*", "*1*", "*1*")")
lt3 = expression("runif("*4*")")
lt4 = expression("rnorm("*4*")")
lt5 = expression(10*"rnorm("*4*")")
lt6 = expression(100*"rnorm("*4*")")
lt7 = expression("Zellner's g-prior")
ltitle = expression(beta[0])

ptitle = expression(paste("Varying Prior Information:  Various ", beta[0]))
...
result_scp1 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=nu0,s20=sig20,gprior=FALSE)
result_scp2 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=nu0,s20=sig2.ols,gprior=FALSE)
result_scp3 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=nu0,s20=10,gprior=FALSE)
result_scp4 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=nu0,s20=100,gprior=FALSE)
result_scp5 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=nu0,s20=1000,gprior=FALSE)
result_scp6 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=nu0,s20=10000,gprior=FALSE)
result_zgp = rpredNormReg(S,Xpred,X,y,nu0=nu0,s20=sig2.ols)

lt1 = expression("var(residuals)" == 6.21)
lt2 = expression(sigma[ols]^2 == 8.54)
lt3 = expression(10)
lt4 = expression(100)
lt5 = expression(1000)
lt6 = expression(10000)
lt7 = expression("Zellner's g-prior")
ltitle = expression(sigma[0]^2)

ptitle = expression(paste("Varying Prior Information:  Scaling ", sigma[0]^2))
...
result_scp1 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=1,s20=sig20,gprior=FALSE)
result_scp2 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=10,s20=sig20,gprior=FALSE)
result_scp3 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=100,s20=sig20,gprior=FALSE)
result_scp4 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=1000,s20=sig20,gprior=FALSE)
result_scp5 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=10000,s20=sig20,gprior=FALSE)
result_scp6 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=Sigma.0,nu0=100000,s20=sig20,gprior=FALSE)
result_zgp = rpredNormReg(S,Xpred,X,y,nu0=nu0,s20=sig2.ols)
...
lt3 = expression(100)
lt4 = expression(1000)
lt5 = expression(10000)
lt6 = expression(100000)
lt7 = expression("Zellner's g-prior")
ltitle = expression(nu[0])

ptitle = expression(paste("Varying Prior Information:  Scaling ", nu[0]))
