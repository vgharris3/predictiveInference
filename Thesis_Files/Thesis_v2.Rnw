...
The plot below shows the density curves of the math scores for these three schools and their predictions.  The observed data is shown using solid lines, and the predictive densities are displayed with dashed lines.

\textcolor{red}{look at the form in which rpredNormIGk() takes in the data.  Ask Dean what he prefers.  Currently taking it as a list of vectors of varying lengths.  Maybe prefers long two-column dataframe with indices in the first column, like the raw data Y.school.mathscore?}
...
for(legInd in 1:length(toplot)){
  nextlegObs = eval(bquote(expression(tilde(y)*" ("*.(toplot[legInd])*")")))
  nextlegPred = eval(bquote(expression(y*" ("*.(toplot[legInd])*")")))
  if(legInd == 1){
...
legend("topright", inset = 0.05,bg="transparent",
       ...
       legend = c(leg[1],leg[2],leg[3],leg[4],leg[5],leg[6]),
       lwd=2,
       # col=c("black","red"),
       col=c(toplot[1],toplot[1],toplot[2],toplot[2],toplot[3],toplot[3]),
       lty=c(1,2,1,2,1,2),
       box.col="white")
...
for(i in 1:10){
  lines(density(result$YTILDE[1001:5000,i]),col=i+1)
  nextleg = eval(bquote(expression(tilde(y)*" (School "*.(i)*")")))
  if(i == 1){
    leg = nextleg
  } else {
    leg = c(leg,nextleg)
...
legend("topright", inset = 0.05,bg="transparent",
       # legend=c(expression(y[toplot]),expression(tilde(y)[toplot])),
       legend = c(expression(y),leg[1],leg[2],leg[3],leg[4],leg[5],leg[6],leg[7],leg[8],leg[9],leg[10]),
       lwd=1,
       # col=c("black","red"),
       col=1:11,
       box.col="white")
