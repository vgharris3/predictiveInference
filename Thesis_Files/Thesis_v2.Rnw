...
@
    ...
    For two-level data consisting of groups and units within groups, denote data $\mathbf{Y}_1,...,\mathbf{Y_m}$ where $\mathbf{Y}_j = \{Y_{1,j},...,Y_{N_j,j}\}$. We have the hierarchical Normal model (Hoff p. 132ff):
    ...
    \end{flalign*}
      ...
      \paragraph{Derivation}
      As with the two-sample problem, joint posterior inferences for the unknown parameters can be made by constructing a Gibbs sampler to approximate the posterior distribution $p\left(\theta_1,...,\theta_m,\mu,\tau^2,\sigma^2|\mathbf{y_1,...,y_m}\right)$.  For this we need the full conditional distribution of each parameter (Hoff pp. 134-135):
      ...
      $$\left\{\sigma^2|\mathbf{\theta,y_1,...,y_m}\right\} \sim \text{Inverse Gamma}\left(\dfrac{1}{2}\left[\nu_0 + \sum_{j=1}^m n_j\right],\dfrac{1}{2}\left[\nu_0\sigma^2_0 + \sum_{j=1}^m\sum_{i=1}^{n_j}\left(y_{i,j}-\theta_j\right)^2\right]\right).$$

      \textcolor{red}{Note:  In the above, Hoff has $\left\{\sigma^2|\mathbf{\theta,y_1,...,y_n}\right\}$. I believe the subscript $n$ is a typo, and should be $m$, the number of observed data groups.}
