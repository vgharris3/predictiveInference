...
Xpred = X#[1:2,]           #predictor vector(s) for which prediction is desired
...
Xpred = t(as.matrix(X[1,]))           #predictor vector(s) for which prediction is desired
Xpred = X
...
# NSamples = c(1000,10000,100000,1000000)
# NSamples = 10000
# par(mfrow = c(1,2))
#
...
#       line_type = j
#       if(first_time){
#         plot(density(result$predictions),ylim=c(0,1.2*max(density(result$predictions)$y)),main=bquote(.(S) ~ "Samples, repeated 3 times"))
#         first_time = 0
#       } else {
#         lines(density(result$predictions),lty=j)
#       }
#     }
#   }
# }
