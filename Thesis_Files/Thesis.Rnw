...
\section{Thesis Abstract}
    ...
    \indent An obstacle to widespread employment of Bayesian predictive inference in scientific research is the lack of suitable computing tools.  In this thesis we document several established useful models, and provide an applicable set of tools for statisticians.  For each of the included models, some basic notes on mathematical derivation are presented, and predictive inference is illustrated with examples.  Note that throughout this thesis the terms ``predictive inference" and ``Bayesian predictive inference" are used interchangeably.  For the details of the models and some of the examples we relied primarily on Seymour Geisser's \underline{Predictive Inference:  An Introduction} (1993) and Peter D. Hoff's \underline{A First Course in Bayesian Predictive Methods} (2009).
...
\textcolor{red}{
  ...
  \textcolor{red}{paragraph or two about the difference between parametric inference and predictive inference}
    ...
    \textcolor{red}{When is predictive inference more useful?}\\
  ...
  Suppose $t=4$ Razorbacks have been observed out of $N=10$ tosses of a single pig die, suggesting a straightforward binomial distribution with $\theta =$ Pr(Razorback) $= 0.4$. Taking the Duquesne experiment into consideration, we'll perform Bayesian prediction using three prior distributions for $\theta$: $\theta\sim\text{Beta}(2,8)$, $\theta\sim\text{Beta}(22,78)$, and $\theta\sim\text{Beta}(224,776)$, and compare these results to predictions obtained from the plug-in estimator.   For $M = 100$ future observations, how many Razorbacks are predicted?  The density curves in the plot below show the influence of the prior on the location of the predictive distribution, and also the difference in variance resulting from the choice of shape parameters used in the Beta prior.  Essentially, each $(\alpha,\beta)$ pair reflects the prior knowledge from the Duquesne experiment, with the ``weight" given to that knowledge increasing by orders of magnitude, causing variance in the prediction to decrease.
...
f_x_PI = dbinom(x,M,t/N)

fmat = cbind(f_x_BB28,f_x_BB2278,f_x_BB224776,f_x_PI)
...
leg4 = eval(bquote(expression("y ~ Binomial("*.(M)*","*.(t/N)*")")))
