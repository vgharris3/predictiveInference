...
by means of Monte Carlo sampling, as follows.
...
for(N in NSamples){
  first_time = 1
  for(j in 1:3){
    result = rpredNormReg(N,X[1,],X,y,nu0,sig2.ols)
    if(first_time){
      plot(density(result),ylim=c(0,1.2*max(density(result)$y)),main=bquote(.(N) ~ "Samples, repeated 3 times"))
      first_time = 0
    } else {
