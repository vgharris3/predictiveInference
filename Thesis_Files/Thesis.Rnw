...
maxx = 0
minx = 0
for(i in 1:nrow(result$predictions)){
...
plot(density(result$predictions[1,]), main = bquote("Predictive Density"),col=1,xlim = c(minx,maxx))

for(i in 2:nrow(result$predictions)){
  lines(density(result$predictions[i,]),col=i)
}

legend("topright",legend=i:nrow(result$predictions),col=i)



# plot(density(result[1,]), main = bquote("Predictive Density of Response for Design Point x ="~"("~.(Xpred[1])~.(Xpred[2])~.(Xpred[3])~.(Xpred[4])~")" ))
# lines(density(result[2,]))
...
# result = rpredNormReg(S,Xpred,X,y,nu0,sig2.ols,gprior=TRUE)
#
# plot(density(result))
...
for(S in NSamples){
  ...
  for(j in 1:3){
#rpredNormReg = function(S=1,Xpred,X,y,beta0,Sigma0,nu0=1,s20=1,gprior = TRUE)
    result = rpredNormReg(S,X[1,],X,y,nu0=nu0,s20=sig2.ols)
    if(first_time){
      plot(density(result$predictions[1,]),ylim=c(0,1.2*max(density(result$predictions[1,])$y)),main=bquote(.(S) ~ "Samples, repeated 3 times"))
      first_time = 0
    } else {
      lines(density(result$predictions[1,]),col=j)
    }
  }
}
