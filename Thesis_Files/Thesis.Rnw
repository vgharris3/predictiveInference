...
\section{Thesis Abstract}
  \begin{itemize}
    ...
    There is a disparity between the growing interest in Bayesian predictive inference and the rate at which these techniques are employed in scientific research.  An obstacle to widespread adoption of predictive inference is the lack of suitable computing tools.  Here we demonstrate methods for several useful cases, and provide an applicable set of tools for statisticians.\\
    The following cases, borrowed from Seymour Geisser and Peter D. Hoff, are the object of this thesis:
    \begin{itemize}
