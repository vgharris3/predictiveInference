...
result_scp1 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=diag(4),nu0=nu0,s20=sig20,gprior=FALSE)
result_scp2 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=diag(diag(Sigma.0)),nu0=nu0,s20=sig20,gprior=FALSE)
result_scp3 = rpredNormReg(S,Xpred,X,y,beta0=beta.ols,Sigma0=diag(rep(1000000,4)),nu0=nu0,s20=sig20,gprior=FALSE)
result_zgp = rpredNormReg(S,Xpred,X,y,nu0=nu0,s20=sig2.ols)

maxx = 0
minx = 0
maxy = 0
numplots = 3#nrow(result_scp$predictions)
for(i in 1:numplots){
  tempmaxx = max(density(result_scp$predictions[i,])$x)
  if(tempmaxx > maxx) {maxx = tempmaxx}
  tempminx = min(density(result_scp$predictions[i,])$x)
...
lines(density(result_scp$predictions[1,]),col=i)
