...
To use the semiconjugate prior, values must be supplied for prior parameters $(\beta_0, \Sigma_0)$ and $(\nu_0, \sigma^2_0)$.  In the example below (Hoff data and code found \href{https://pdhoff.github.io/book/}{here}), we use


\begin{flalign*}
  \beta_0 &= \hat{\beta}_{ols} \text{ (ordinary least squares estimator of } \beta \text{)}\\
  \Sigma_0 &= (X^TX)^{-1}\sigma^2n \text{ (sampling variance of } \hat{\beta}_{ols} \text{)}\text{WHY TIMES n????}\\
  \nu_0 &= 1 \text{ (prior sample size)}\\
  \sigma^2_0 &= \frac{\sum e_i}{n-1} \text{ (variance of the residuals)}
\end{flalign*}
...
#set.seed(1)
result = rpredNormReg(S,Xpred,X,y,beta.ols,Sigma.0,nu.0,sig20,gprior=FALSE)
plot(density(result))
...
#   first_time = 1
#   for(j in 1:3){
#     result = rpredNormReg(S,X[1,],X,y,beta.ols,Sigma.0,nu.0,sig20,gprior=FALSE)
#     if(first_time){
#       plot(density(result),ylim=c(0,1.2*max(density(result)$y)),main=bquote(.(S) ~ "Samples, repeated 3 times"))
#       first_time = 0
#     } else {
#       lines(density(result),col=j)
#     }
#   }
# }
