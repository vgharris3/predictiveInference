...
\textcolor{red}{BELOW ARE JUST EXAMPLES OF MINIMIZING SQUARED RESIDUALS UNDER VARIOUS ASSUMPTIONS ABOUT THE BETAS}
...
textcolor{red}{need to understand this better.  How are the values of the ols beta estimates being compared to their standard errors?  Seems like -51 is 4+ standard errors away from 0, for example}
...
\clearpage
  ...
  \subsubsection{Derivation}
    ...
    Hoff proposes a semiconjugate prior distribution for $\beta$ and $\sigma^2$ to be used when there is information available about the parameters.  The sampling density of the data (Equation \ref{conditional_density}) is
...
Simple Monte Carlo approximation can be used to sample from the joint posterior density $p(\sigma^2,\beta|\mathbf{y,X})$ as follows.  Here $g$ is typically set to the number of prior observations.  Then:
...
The standard format R function rpredNormReg() approximates the joint posterior density $p(\sigma^2,\beta|\mathbf{y,X})$ using one of the two methods described above, as selected by the user.  The user inputs the set or sets of explanatory variables $X_{pred} = (x_{i1}, x_{i2}, x_{i3}, x_{i4})$ and the number $S$ of predicted values desired for each set. The function produces $S$ sampled values from the relevant posterior distribution of $\beta$, and then computes a prediction for each beta as $y = X_{pred}\boldsymbol\beta$.
...
\begin{flalign*}
  \beta_0 &= \hat{\beta}_{ols} = (\Sexpr{beta.0.disp}, \Sexpr{beta.0.disp}, \Sexpr{beta.0.disp}, \Sexpr{beta.0.disp})  \text{ (ordinary least squares estimator of } \beta \text{)}\\
  \Sigma_0 &= (X^TX)^{-1}\sigma^2n =
    ...
    \end{pmatrix}
    \text{ (sampling variance of } \hat{\beta}_{ols} \text{)}\text{WHY TIMES n????}\\
