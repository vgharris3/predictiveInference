...
\section{Thesis Abstract}
  \begin{itemize}
    ...
    An obstacle to widespread adoption of predictive inference is the lack of suitable computing tools.  Here we demonstrate Bayesian predictive methods for several
...
\section{Introduction:  Predictive Inference}
  ...
  This thesis and the accompanying set of tools (R package) employs a Bayesian approach to predictive inference relying on the de Finetti representation theorem (de Finetti 1937). \textcolor{red}{State theorem here?} We can write

\begin{equation}
  g(x_{n+1}|\mathbf{x}_n) = \frac{\int_\Theta f(\mathbf{x}_{n+1}|\mathbf{\theta})\pi(\mathbf{\theta})d\mathbf{\theta}}{\int_\Theta f(\mathbf{x}_n|\mathbf{\theta})\pi(\mathbf{\theta})d\mathbf{\theta}} = \frac{h(\mathbf{x}_{n+1})}{h(\mathbf{x}_n)} \label{generalPredictiveDistribution}
\end{equation}
