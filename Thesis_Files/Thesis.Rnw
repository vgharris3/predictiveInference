...
minx = 0
maxy = 0
numplots = 7#nrow(result_scp$predictions)
...
# tempmaxx = max(density(result_scp1$predictions[1,])$x)
# if(tempmaxx > maxx){maxx = tempmaxx}
# tempmaxx = max(density(result_scp2$predictions[1,])$x)
# if(tempmaxx > maxx){maxx = tempmaxx}
# tempmaxx = max(density(result_scp3$predictions[1,])$x)
# if(tempmaxx > maxx){maxx = tempmaxx}
# tempmaxx = max(density(result_scp4$predictions[1,])$x)
# if(tempmaxx > maxx){maxx = tempmaxx}
# tempmaxx = max(density(result_scp5$predictions[1,])$x)
# if(tempmaxx > maxx){maxx = tempmaxx}
# tempmaxx = max(density(result_scp6$predictions[1,])$x)
# if(tempmaxx > maxx){maxx = tempmaxx}
# tempmaxx = max(density(result_zgp$predictions[1,])$x)
# if(tempmaxx > maxx){maxx = tempmaxx}

plot(density(result_scp1$predictions[1,]), main = expression(paste("Varying Prior Information:  ", Sigma[0])),col=c(rep(1,6),2),xlim = c(minx,1.25*maxx),ylim = c(0,1.1*maxy))

#PLOT DIFFERENT RESULTS FOR THE DIFFERENT E MATRICES.  THEN DO THIS VARYING THE OTHER PRIOR INFO INPUTS.
...
# }
#
#legtxt = c(expression((X^T*X)^-1*sigma^2),expression(paste("diag"((X^T*X)^-1*sigma^2)) ))
legtxt = c(lt1,lt2,lt3,lt4,lt5,lt6,lt7)
legend(0.4*maxx,maxy,legend = legtxt,col=c(rep(1,6),2),lty = 1:7,lwd=c(rep(1,6),2),bty = "n")
# legend("topright",legend = legtxt,lty = 1:2,bty = "n")

# for(i in 1:numplots){
  # lines(density(result_zgp$predictions[1,]),lty=2,col=i)
# }
