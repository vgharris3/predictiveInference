...
plot(0:M,f_x,type = "l", col = "blue", lwd = 2, main = "Beta-Binomial Predictive Density")

#Cumulative predictive probability
F_x = ppredBB(0:M,N,t,M,a,b)
plot(0:M,F_x,type = "l", col = "red", lwd = 2, main = "Beta-Binomial Cumulative Predictive Probability")

#Random sample
f_x = dpredBB(0:M,N,t,M,a,b)
random_sample = rpredBB(1000,N,t,M,a,b)
hist(random_sample,prob = T, main = "Histogram of Sample with Density Curve Overlay",ylim = c(0,max(f_x)*1.5),xlim = c(0,M))
lines(0:M,f_x)
@
...
#Predictive density
f_x = dpredPG(obs, ytilde, alpha, beta)
  ...
  \subsection{Normal Observation with Normal-Inverse Gamma Prior}
    \subsubsection{One sample}
      ...
      [Hoff p. 69ff]\\
        ...
        $$\left\{1/\sigma^2|y_1,...,y_n\right\}\sim gamma\left(\frac{n}{2},\frac{n}{2}\frac{1}{n}\sum\left(y_i - \bar{y}\right)^2\right)$$

        same as

        $$\left\{1/\sigma^2|y_1,...,y_n\right\}\sim gamma\left(\frac{n}{2},\frac{1}{2}\sum\left(y_i - \bar{y}\right)^2\right)$$



        $$\left\{\theta|\sigma^2, y_1,...,y_n\right\}\sim normal\left(\bar{y},\frac{\sigma^2}{n}\right)$$

        \textcolor{red}{dig into Bedrick notes and homework for Jeffrey's prior}

<<fig=TRUE, echo=FALSE>>=
#y taken from Bedrick's Bayesian Lecture 9 Gibbs Sampler example
