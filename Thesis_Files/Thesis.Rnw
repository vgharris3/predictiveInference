  ...
  \subsection{Normal Observation with Normal-Inverse Gamma Prior}
    \subsubsection{One sample}
      ...
      [Hoff p. 69ff]\\
        ...
        Here $\bar{y} = \frac{1}{n}\sum_{i=1}^n y_i$ is the sample mean and $s^2 = \sum_{i=1}^n\frac{\left(y_i - \bar{y}\right)^2}{(n-1)}$ is the sample variance.\\
        ...
        \textcolor{red}{WHAT ABOUT BURN-IN?  HOW MANY TO DISCARD?}
...
for(yInd in 1:S){
  ytilde[yInd] = rnorm(1,theta.postsample,sqrt(sig2.postsample))
}
