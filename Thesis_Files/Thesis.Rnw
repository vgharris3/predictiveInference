...
\noindent where $x_{n+1}$ is conditionally independent of $\mathbf{x}_n$ given $\boldsymbol\theta$, and $\pi(\boldsymbol\theta | \mathbf{x}_n)d\boldsymbol\theta)$ is the posterior distribution of $\mathbf{x}_n$ given $\boldsymbol\theta$.
  ...
  Suppose $t=4$ Razorbacks have been observed out of $N=10$ tosses of a single pig die, suggesting a straightforward binomial distribution with $p =$ Pr(Razorback) $= 0.4$. Taking the Duquesne experiment into consideration, we'll assign prior $p\sim\text{Beta}(22,78)$ and compare resulting predictions to those obtained from the plug-in estimator.   For $M = 100$ future observations, how many Razorbacks are predicted?  The differences in the density curves (below) are striking, with the experimental mean number of Razorbacks very unlikely to be predicted by the binomial using the plug-in value of $0.4$.
...
matplot(x,fmat,type="l",lty=1,lwd = 2, col = c(1,2), ylim = c(0,ymax),
     main = expression("Pass the Pigs"^"TM"*":  Beta-Binomial Prediction vs. Binomial Plug-in"))
