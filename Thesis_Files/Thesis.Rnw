...
\noindent where $\nu_0$ as before is the assumed prior sample size.  The full conditional distributions follow:\\
      ...
      The results of a call to rpredNormIG2$\left(\mathbf{y}_1,\mathbf{y}_2,\mu_0,\sigma^2_0,\delta_0,\tau^2_0,N\right)$ are summarized in the following plots.\\

      NOW WRITE RPREDNORMIG2()
...
for(s in 1:5000)
{
  ...
  ##update mu
  var.mu<-  1/(1/g02+ (n1+n2)/s2 ) #this is gamma^2_n -- call it g2n
  mean.mu<- var.mu*( mu0/g02 + sum(y1-del)/s2 + sum(y2+del)/s2 )  #this is mu_n -- call it mun
...
plot(1:n1,y1)
points(1:n2,y2,col="red")
abline(h=Y12[,1])
abline(h=Y12[,2],col="red")
plot(density(y1))
lines(density(y2),col="red")
lines(density(Y12[,1]))
lines(density(Y12[,2]),col="red")
@
