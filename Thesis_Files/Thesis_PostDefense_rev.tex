...
\input{Thesis_PostDefense_rev-concordance}
...
\noindent For the purpose of this example, consider the probability of a single pig landing in the ``Razorback" position, which occurs when the pig is lying on its back with its legs extended upward.  The irregular shape of the pig makes it difficult to assign probabilities to results other than by means of experimentation.  Such an experiment was conducted at Duquesne University, and an article describing the experiment as well as Bayesian predictive inference performed on the results appeared in the \textit{Journal of Statistics Education} Volume 14, Number 3, in 2006 [7].  The article can be accessed here:  \url{http://jse.amstat.org/v14n3/datasets.kern.html}. Of the $11,954$ recorded results for individual pigs, approximately $22.4\%$ were Razorbacks.\\
...
\begin{center}
  ...
  \multicolumn{1}{c}{\textbf{Prediction Method}} & \multicolumn{1}{c}{$\textbf{(}\boldsymbol\alpha,\boldsymbol\beta\textbf{)}$}  & \multicolumn{1}{c}{\textbf{Prior Mean }$\textbf{E(}\boldsymbol\theta\textbf{)}$}  & \multicolumn{1}{c}{\textbf{Prediction Mean}} & \multicolumn{1}{c}{\textbf{SD}}\\
        \midrule
        Beta-Binomial & (2,8) & 0.2 & 30.1 & 11.09 \\
        \midrule
        Beta-Binomial & (22,78) & 0.22 & 23.41 & 5.63\\
        \midrule
        Beta-Binomial & (224,776) & 0.224 & 22.65 & 4.35 \\
        \midrule
...
size, and $S$ is the desired number of draws from the predictive distribution. The predicted future result
...
\begin{flalign*}
  ...
  \texttt{S} &= \text{the desired number of draws from the predictive distribution; that is, the number of future experiments }\\
...
\noindent The user provides the two samples $\mathbf{y_1}$ and $\mathbf{y_2}$ along with values for $\mu_0, \sigma^2_0, \delta_0, \tau^2_0, \nu_0$, $S$.  The function returns $S$ draws from the predictive distribution for each of the two populations along with the vectors of generated values for $\mu$, $\delta$, and $\sigma^2$.  Calls to this functions appear as follows:
...
\begin{flalign*}
  \texttt{S} &= S \text{, the number of draws from the predictive joint distribution}\\
...
\begin{flalign*}
  \texttt{S} &=S \text{, the number of draws from the predictive joint distribution. The sampler returns } S \text{ realizations}\\
  &\text{from the predictive distribution, each having } k \text{ elements, one corresponding to each}\\
  &\text{data group } \mathbf{Y}_1,...,\mathbf{Y_k} \text{. That is, } \mathbf{\tilde{Y}_j} = \tilde{Y}_{1,j},...,\tilde{Y}_{s,j} \text{, for } j \in \{1,...,k\}, s\in\{1,...S\}.\\
...
\clearpage
  ...
  \subsubsection{Derivation (Normal Regression)}
    ...
    % $$p(\mathbf{y}|\mathbf{X},\boldsymbol\beta,\sigma^2) \propto \text{exp}\{-\frac{1}{2\sigma^2}\text{SSR}(\boldsymbol\beta)\} = \text{exp}\{-\frac{1}{2\sigma^2}[\mathbf{y}^T\mathbf{y} - 2\boldsymbol\beta^T\mathbf{X}^T\mathbf{y}+\boldsymbol\beta^T\mathbf{X}^T\mathbf{X}\boldsymbol\beta]\}\text{ (Hoff)},$$
    %
    %  OR

\begin{flalign*}
  p(\mathbf{y}|\mathbf{X},\boldsymbol\beta,\sigma^2) &\propto \left(\sigma^2\right)^{-0.5(N-p)}\text{exp}\{-\frac{1}{2\sigma^2}\text{SSR}(\boldsymbol\beta)\} \\
  &= \left(\sigma^2\right)^{-0.5p}\text{exp}\{-\frac{1}{2\sigma^2}[\mathbf{y}^T\mathbf{y} - 2\boldsymbol\beta^T\mathbf{X}^T\mathbf{y}+\boldsymbol\beta^T\mathbf{X}^T\mathbf{X}\boldsymbol\beta]\}\text{ (Bedrick)}
\end{flalign*}


\noindent and for priors we choose

$$\boldsymbol\beta \sim \text{Multivariate Normal}(\boldsymbol\beta_0,\Sigma_0)$$

and

$$1/\sigma^2 = \gamma\sim \text{Gamma}(\nu_0/2,\nu_0\sigma^2_0/2).$$

\noindent Note that these prior distributions are apriori independent.  We have conditional posterior for $\boldsymbol\beta$
...
\noindent To create a realization from the predictive distribution of responses:  for each $s\in\{1,...,S\}$, draw $\epsilon^{(s)} \sim \text{Normal}(0,\sigma^{2(s)})$.  Then compute
...
\begin{flalign*}
  \texttt{S} &= S \text{, the number of MCMC realizations (the desired number of draws from the predictive distribution)}\\
...
\begin{flalign*}
  ...
  S &= 5000 \text{, the number of draws from the predictive distribution}
\end{flalign*}
...
\bibitem{Kern} Kern, John (2006), ``Pig Data and Bayesian Inference on Multinomial Probabilities," Journal of Statistics Education, Volume 14, Number 3.
\end{thebibliography}
