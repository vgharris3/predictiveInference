...
matplot(x,fmat,type="l",lty=c(1,2,3,1),lwd = 2, col = c(1,1,1,2), ylim = c(0,ymax),
     main = expression("Pass the Pigs"^"TM"*":  Beta-Binomial Prediction vs. Binomial with Plug-in Estimator"),
...
\begin{center}
  \begin{tabular}{ |c|c|c|c| }
     ...
     \hline
     (\Sexpr{a1},\Sexpr{b1}) & \Sexpr{a1/(a1+b1)} & \Sexpr{round(PtPMean2_8,2)} & \Sexpr{round(PtPSD2_8,2)} \\
     \hline
     (\Sexpr{a2},\Sexpr{b2}) & \Sexpr{a2/(a2+b2)} & \Sexpr{round(PtPMean22_78,2)} & \Sexpr{round(PtPSD22_78,2)} \\
     \hline
     (\Sexpr{a3},\Sexpr{b3}) & \Sexpr{a3/(a3+b3)} & \Sexpr{round(PtPMean224_776,2)} & \Sexpr{round(PtPSD224_776,2)} \\
     \hline
  \end{tabular}
\end{center}
  ...
  We want to predict future outcomes based on current knowledge.  Specifically we're asking: for observed values $Y_1 = y_1,...,Y_n = y_n$, what is likely to be the value of the next observation, $\tilde{Y}=\tilde{y}$?  We want to compute $Pr(\tilde{Y}|\theta,y_1,...,y_n)$, where $\theta$ is a population parameter with a distribution $\pi(\theta)$ based on some prior knowledge or beliefs.  Here we are careful to satisfy ourselves that $Y_1,...,Y_n$ are \textit{exchangeable}, which enables us to rely on de Finetti's representation theorem for the conditional independence and identical distribution of $Y_1,...,Y_n$.  Thus we can write
...
For prediction, we need only to characterize the observed data $(y_1,...,y_n)$ conditional on the relevant parameter(s) $(\theta)$ and supply a suitable prior distribution for the relevant parameter(s) $\theta$.  From there we compute
...
and make our prediction using (\ref{BayesianPredictiveFormat}).
  ...
  % Exchangeability means that $p(y_1,...,y_n) = p(y_{a_1},...,y_{a_n})$ for all permutations $\{a_1,...,a_n\}$ of $\{1,...,n\}$.  In other words the order of the $y_i$s does not convey any information about their joint density.  From de Finetti's representation theorem, then, $Y_1,...,Y_n$ are conditionally independent and identically distributed (i.i.d.) given $\theta$ (Hoff p. 27), and we can write the joint density
  %
  % $$p(y_1,...,y_n) = \int\left\{\prod_1^n p(y_i|\theta)\right\}\pi(\theta)d\theta$$.
  %
...
%     \textbf{Definition} (Hoff p. 27) Let $p(y_1,...,y_n)$ be the joint density of $Y_1,...,Y_n$.  If $p(y_1,...,y_n) = p(y_{\pi_1},...,y_{\pi_n})$ for all permutations $\{\pi_1,...,\pi_n\}$ of $\{1,...,n\}$, then $Y_1,...,Y_n$ are \textit{exchangeable}.
%
%   Given joint density $p(y_1,...,y_n)$ of $Y_1,...,Y_n$ and some parameter $\theta$ with prior distribution $\pi(\theta)$, de Finetti' Representation Theorem (1937) enables us to write
%
%   $$p(y_1,...,y_n) = \int$$
%
%   This thesis and the accompanying set of tools (R package) employs a Bayesian approach to predictive inference relying on the de Finetti representation theorem (de Finetti 1937).
%
% %   ``Roughly speaking, $Y_1,...,Y_n$ are exchangeable if the subscript labels convey no information about the outcomes."
% %
% % The examples presented in this thesis start with an assumption of exchangeability.
%
%
%   \textcolor{red}{State the de Finetti theorem here?} We can write
%
% \begin{equation}
...
maxy = 1.5*max(density(Y[[toplot]])$y)
plot(density(Y[[toplot]]),col="black",main="School 1 Data and Prediction",
     xlab="Grade",ylim=c(0,maxy))

for(i in 1:1){
  result = rpredNormIGk(N=5000,Y,nu0=1,s20=100,eta0=1,t20=100,mu0=50,g20=25)
  lines(density(result$YTILDE[1001:5000,1]),col="red")
}
