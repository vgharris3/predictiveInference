...
\textcolor{red}{NOTE:  in the book Hoff defines $X$ as a matrix whose \textit{rows} are the vectors $x_1,...,X_n$ whereas in the calculation we need the \textit{columns} to be $x_1,...,x_n$.}\\
